...
/* hxlin fixed */
__constant__ float ellipsoid_x;
__constant__ float ellipsoid_y;
__constant__ float ellipsoid_offset[2];
/* hxlin fixed */

__constant__ float normal_radius;
__constant__ float max_phx;
__constant__ float max_phy;
__constant__ float max_curvature;
__constant__ float weight_phi;
...
__device__ void computerEigenvalue(float *pMatrix, int nDim, float *maxvector, float *curvature, float dbEps, int nJt)
{
    ...
    for(int i = 0; i < nDim; i ++) 
    {  
        maxvector[i] = pdblVects[min_id + nDim * i];  // compute  normal_vec(zb)
    }

    *curvature = 3.0 * minEigenvalue / sumEigenvalue;  // compute curvature
}
...
/*   SE2 Methods for planner   origin */
                                ...
                                )
{
    ...
    if (se2_idx < xyyaw_num)
    {
        ...
        // for col-major
        /*Hxlin try to debug for cnt_p*/
        int map_idx_true = map_idx / size_pos[0] + (map_idx % size_pos[0]) * size_pos[1]; //origin
        // int map_idx_true = map_idx / size_pos[1] + (map_idx % size_pos[1]) * size_pos[0];
        ...
        if (!map_nan[map_idx_true] || inpaint_method!=0)
        {
            ...
            // {
                // if (iter != 0)
                // {
                //     zb[0] = zbx[se2_idx_true];
                //     zb[1] = zby[se2_idx_true];
                //     zb[2] = sqrt(1.0-zb[0]*zb[0]-zb[1]*zb[1]);  
                // }
            // yb = zb.cross(xyaw).normalized();
            // xb = yb.cross(zb);
            Array2f ellipseHalfLength;
            Vector2f ellipseCenter = gridCenter + ellipsoid_offset[0] * xb.head(2) + ellipsoid_offset[1] * yb.head(2);
            ellipseHalfLength(0) = ellipsoid_x * sqrt(1.0 - xb(2)*xb(2)) * 0.5;
            ellipseHalfLength(1) = ellipsoid_y * sqrt(1.0 - yb(2)*yb(2)) * 0.5;
            Array2f semiAxisSquare_(ellipseHalfLength[0]*ellipseHalfLength[0], ellipseHalfLength[1]*ellipseHalfLength[1]);

            // AABB method
            Vector2f u(xyaw[0] * ellipseHalfLength[0], xyaw[1] * ellipseHalfLength[0]);
            Vector2f v(-xyaw[1] * ellipseHalfLength[1], xyaw[0] * ellipseHalfLength[1]);
            Vector2f poses[3];
            poses[0] = ellipseCenter-u-v;
            poses[1] = ellipseCenter-u+v;
            poses[2] = ellipseCenter+u-v;
            Vector2f top_left = ellipseCenter+u+v;
            ...
            Vector3f temp_poses[100];
                /*hxlin debug*/
                // printf("buffer_size:(%d,%d)",buffer_size[0],buffer_size[1]);
            ...
            {   
                for (int j=0; j<buffer_size[1]; j++)
                {
                    ...
                    if (!map_nan[true_idx] || inpaint_method!=0)
                    {
                        ...
                        temp_pos = temp_pos - ellipseCenter;
                        Array2f value;
                        value[0] = transformMatrix_.row(0).dot(temp_pos); 
                        value[1] = transformMatrix_.row(1).dot(temp_pos); 
                        value[0] = value[0] * value[0] / semiAxisSquare_[0];
                        value[1] = value[1] * value[1] / semiAxisSquare_[1];
            ...
            if (yaw_idx==0)
                ...
                // printf("hxlin Debug so cnt_p =%d\n",cnt_p);
            if (cnt_p > 7)
            {   
                //printf("Hxlin Debug enter cnt_p >>>>>7!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
                float pMatrix[9] = {0};
                ...
                /* hxlin fixed */
                float curvature;
                computerEigenvalue(pMatrix, 3, normal_vec, &curvature, dbEps, nJt);  //CovMatrix —> curvature,normal_vec
                // float cos_xi;
                /* hxlin fixed */
                if (normal_vec[2] > 0)  //用地形法向量更新 zbx,zby
                {
                ...
                else
                {
                    ...
                    // float sin_testphi = -1.0;
                    // float testphi = std::asin(sin_testphi);
                    // printf("test_phi:%.4f\n",testphi);


                    float sin_phx,sin_phy,phx,phy;
                    sin_phx = Vector3f::UnitZ().dot(xb);
                    sin_phy = Vector3f::UnitZ().dot(yb);

                    phx= abs(std::asin(sin_phx));
                    phy= abs(std::asin(sin_phy));
                    // printf("test_phi:phx:%.4f,phy:%.4f \n",max_phx,max_phy);
                    // printf("test_phi:pi:%.4f\n",M_PI/4);
                    if (phx > max_phx || phy > max_phy )
                    {
                        // printf("test_phi:phx:%.4f,phy:%.4f \n",phx,phy);
                        risk[se2_idx_true] = 1.0;
                        sdf[map_idx_true] = 1.0;
                    ...
                    {
                    float risk_cur_= curvature/max_curvature;
                    
                    float risk_phx_= phx/max_phx;
                    float risk_phy_= phy/max_phy;


                    Vector3f risk_(risk_cur_, risk_phx_, risk_phy_);
                    Vector3f weight_risk_ (weight_curvature, weight_phi, weight_phi);
                    risk[se2_idx_true] = risk_.dot(weight_risk_);
                    // printf("risk_cur_:%f,risk:%f",curvature,risk[se2_idx_true]);
            ...
            const Vector3f& body2sensor_T_)
{
    ...
    /* hxlin fixed */
    cudaMemcpyToSymbol(ellipsoid_x, &ellipsoid_x_, sizeof(float));
    cudaMemcpyToSymbol(ellipsoid_y, &ellipsoid_y_, sizeof(float));
    cudaMemcpyToSymbol(ellipsoid_offset, &ellipsoid_offset_, sizeof(float) * 2);
    /* hxlin fixed */
    
    cudaMemcpyToSymbol(normal_radius, &normal_radius_, sizeof(float));
    cudaMemcpyToSymbol(max_phx, &max_phx_, sizeof(float));
    cudaMemcpyToSymbol(max_phy, &max_phy_, sizeof(float));
    cudaMemcpyToSymbol(max_curvature, &max_curvature_, sizeof(float));
    cudaMemcpyToSymbol(weight_phi, &weight_phi_, sizeof(float));
    ...
    /* hxlin fixed */
    cudaMemcpyToSymbol(iter_num, &iter_num_, sizeof(float));
    /* hxlin fixed */
                    ...
                    bool verbose_)
{
    ...
    cudaDeviceSynchronize(); // Hxlin debug for sdf
    t1 = clock();
    // printf("get map=%fms\n", (float)(t1-t0)/1000.0);
