 ...
 * @tparam MultiArrayMessageType_ a std_msgs::msg::xxxMultiArray message.
...
bool isRowMajor(const MultiArrayMessageType_& message)
{
  ...
  RCLCPP_ERROR(rclcpp::get_logger("se2_grid_ros"), "isRowMajor() failed because layout label is not set correctly.");
  return false;
}
...
/*!
 * Copies an Eigen matrix into a ROS2 MultiArray message.
 * Both column- and row-major matrices are allowed, and the type
 * will be marked in the layout labels.
 * @tparam MultiArrayMessageType_ a std_msgs::msg::xxxMultiArray message.
 * @tparam EigenType_ an Eigen matrix with matching Scalar type.
 * @param[in] e the Eigen matrix to be converted.
 * @param[out] m the ROS2 message to which the data will be copied.
...
/**
 * Copies a ROS2 xxxMultiArray message into an Eigen matrix.
 * @tparam MultiArrayMessageType_ a std_msgs::msg::xxxMultiArray message.
 * @tparam EigenType_ an Eigen matrix with matching Scalar type.
 * @param[in] m the ROS2 message to which the data will be copied.
...
bool multiArrayMessageCopyToMatrixEigen(const MultiArrayMessageType_& m, EigenType_& e)
{
  if (e.IsRowMajor != isRowMajor(m)) {
    RCLCPP_ERROR(rclcpp::get_logger("se2_grid_ros"), "multiArrayMessageToMatrixEigen() failed because the storage order is not compatible.");
    return false;
  }
...
/*!
 * Maps a ROS2 xxxMultiArray message into an Eigen matrix.
 * Both column- and row-major message types are allowed.
 * @param[in] m the ROS2 message to be converted.
